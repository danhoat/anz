...
class BackWPup_Pro_Settings_APIKeys {
    ...
    const OPTION_ONEDRIVE_CLIENT_ID = 'backwpup_cfg_onedriveclientid';
    const OPTION_ONEDRIVE_CLIENT_SECRET = 'backwpup_cfg_onedriveclientsecret';
	...
	private function __construct() {
        ...
        if ( isset( $destinations['ONEDRIVE'] ) ) {
            add_action( 'backwpup_page_settings_tab_apikey', array( $this, 'onedrive_keys_form' ) );
        }
    ...
    /**
     * Prints form fields for OneDrive keys
     */
    public function onedrive_keys_form() {

        $secret = BackWPup_Encryption::decrypt( get_site_option( self::OPTION_ONEDRIVE_CLIENT_SECRET ) );
        ?>
        <h3><?php esc_html_e( 'OneDrive API Keys', 'backwpup' ); ?></h3>
        <p>
            <a href="https://docs.microsoft.com/en-us/onedrive/developer/rest-api/getting-started/?view=odsp-graph-online">
                <?php esc_html_e( 'Getting started with OneDrive API', 'backwpup' ) ?>
            </a>
        </p>
        <table class="form-table">
            <tr>
                <th scope="row"><label for="onedriveclientid"><?php esc_html_e( 'Client ID:', 'backwpup' ); ?></label>
                </th>
                <td>
                    <input
                        ...
                        type="text"
                        id="onedriveclientid"
                        value="<?php echo esc_attr( get_site_option( self::OPTION_ONEDRIVE_CLIENT_ID ) ); ?>"
                        class="regular-text"
                        autocomplete="off"/>
                </td>
            </tr>
            <tr>
                <th scope="row"><label for="onedriveclientsecret"><?php esc_html_e( 'Client secret:',
                            'backwpup' ); ?></label></th>
                <td>
                    <input
            ...
            <tr>
                <th scope="row"><?php esc_html_e( 'Redirect URI:', 'backwpup' ); ?></th>
                <td>
					<span class="code"><?php echo home_url( 'wp-load.php' ); ?></span>
                </td>
            </tr>
        </table>
        <?php
	...
	public function save_form() {
        ...
        if ( $_POST['onedriveclientsecret'] ) {
            update_site_option(
                self::OPTION_ONEDRIVE_CLIENT_SECRET,
                BackWPup_Encryption::encrypt( sanitize_text_field( $_POST['onedriveclientsecret'] ) )
            );
        } else {
            delete_site_option( self::OPTION_ONEDRIVE_CLIENT_SECRET );
        }

        if ( $_POST['onedriveclientid'] ) {
            update_site_option( self::OPTION_ONEDRIVE_CLIENT_ID, sanitize_text_field( $_POST['onedriveclientid'] ) );
        } else {
            delete_site_option( self::OPTION_ONEDRIVE_CLIENT_ID );
        }
