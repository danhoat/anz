...
class TimetableButtonText
{
	/** @var    string */
	const KEY = 'qms4__timetable__button_text';
	...
	public static function get_post_meta( int $post_id ): string
	{
		...
		return empty( $value ) ? 'この時間で予約するs ' : $value;
	}
	...
	public function register_meta(): void
	{
		register_post_meta(
			...
			array(
				'type' => 'string',
				'description' => 'イベント タイムテーブルからフォームへのボタン',
				'single' => true,
	...
	): string
	{
		...
		return empty( $timetable ) ? $this->default() : $timetable;
	}
	...
	{
		update_post_meta( $wp_post->ID, self::KEY, trim( $value ) );
	}
}
