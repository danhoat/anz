...
<div class="l-event-rec l-other_article">
  <div class="wp-block-group__inner-container c-postContent">
    ...
    <h2 class="has-text-align-center is-style-ja_title-heading">その他のプランを見る</h2>
  </div>
  <ul class="box-list">
<?php if (ok($next_item)) { ?>
...
<?php if (!is_empty($next_item->price)) { ?>
      <div div class="qms4__post-list__post-price p-postList__price"><?= $next_item->price ?></div>
<?php } ?>
        </div>
      </div>
    </a>
...
<?php if (!is_empty($prev_item->price)) { ?>
          <div div class="qms4__post-list__post-price p-postList__price"><?= $prev_item->price ?></div>
<?php } ?>
        </div>
      </div>
    </a>
