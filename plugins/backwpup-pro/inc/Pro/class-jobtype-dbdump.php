...
class BackWPup_Pro_JobType_DBDump extends BackWPup_JobType_DBDump {
	...
	public function option_defaults() {
		...
		$defaults = array(
			...
			'dbdumpdbcharset'    => defined( 'DB_CHARSET' ) ? DB_CHARSET : 'utf8mb4',
			'dbdumpmysqlfolder' => 'mysqldump'
		);
            ...
            <td>
                ...
                <table id="dbconnection"<?php if ( BackWPup_Option::get( $jobid, 'dbdumpwpdbsettings' ) ) echo ' style="display:none;"';?>>
                    <tr><td>
                        ...
                        <input class="text" type="text" id="dbdumpdbhost" name="dbdumpdbhost" autocomplete="off" /></label><br/>
                        <label for="dbdumpdbuser"><?php esc_html_e( 'User:', 'backwpup' );?><br/>
                        <input class="text" type="text" id="dbdumpdbuser" name="dbdumpdbuser" autocomplete="off" /></label><br/>
                        <label for="dbdumpdbpassword"><?php esc_html_e( 'Password:', 'backwpup' );?><br/>
                        <input class="text" type="password" id="dbdumpdbpassword" name="dbdumpdbpassword" autocomplete="off" /></label>
                    </td><td>
	...
	public function edit_form_post_save( $id ) {
		...
		$db_credentials = filter_input_array( INPUT_POST, array(
			'dbdumpdbhost' => FILTER_SANITIZE_STRING,
			'dbdumpdbuser' => FILTER_SANITIZE_STRING,
			'dbdumpdbpassword' => FILTER_SANITIZE_STRING,
			'dbdumpdbname' => FILTER_SANITIZE_STRING,
			'dbdumpdbcharset' => FILTER_SANITIZE_STRING,
		) );

		if (
			! empty( $db_credentials[ 'dbdumpdbhost' ] )
			&& ! empty( $db_credentials[ 'dbdumpdbuser' ] )
			&& ! empty( $db_credentials[ 'dbdumpdbpassword' ] )
			&& ! empty( $db_credentials[ 'dbdumpdbname' ] )
			&& ! empty( $db_credentials[ 'dbdumpdbcharset' ] )
		) {
			BackWPup_Option::update( $id, 'dbdumpdbhost', $db_credentials[ 'dbdumpdbhost' ] );
			BackWPup_Option::update( $id, 'dbdumpdbuser', $db_credentials[ 'dbdumpdbuser' ] );
			BackWPup_Option::update( $id, 'dbdumpdbpassword', BackWPup_Encryption::encrypt( $db_credentials[ 'dbdumpdbpassword' ] ) );
			BackWPup_Option::update( $id, 'dbdumpdbname', ( ! isset( $db_credentials[ 'dbdumpdbname' ] ) ) ? '' : trim( $db_credentials[ 'dbdumpdbname' ] ) );
			BackWPup_Option::update( $id, 'dbdumpdbcharset', $db_credentials[ 'dbdumpdbcharset' ] );
		}
	...
	public function job_run( BackWPup_Job $job_object ) {
		...
		if ( $job_object->job[ 'dbdumpwpdbsettings' ] ) {
			...
			$job_object->job[ 'dbdumpdbcharset' ] = defined( 'DB_CHARSET' ) ? DB_CHARSET : 'utf8mb4';
	...
	public function edit_ajax( $args = array() ) {
		...
		elseif ( $args[ 'action2' ] === 'databases' ) {
			...
			if (
				empty( $args[ 'dbhost' ] )
				|| empty( $args[ 'dbuser' ] )
				|| empty( $args[ 'dbpassword' ] )
			) {
				echo '<span id="dbdumpdbname" class="bwu-message-error">' . esc_html__( 'Please enter database credentials.', 'backwpup' ) . '</span>';
				if ( $ajax )
					die();
				else
					return;
